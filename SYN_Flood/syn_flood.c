...
/* CRC16校验 */
unsigned short cal_checksum (unsigned short *buffer, unsigned short size)     
{  
...
{ 
	...
	while(alive)
	{
		...
		ip.checksum = cal_checksum((u_short *) buf, sizeof(struct ip));
		...
		tcp.sum = cal_checksum((u_short *) buf, sizeof(pseudoheader)+sizeof(struct tcphdr));
